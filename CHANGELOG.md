...
> 「为什么要限制导师说话的字数?不是很明白」
>
> 「我觉得回复的质量才是最重要,而不是强制要求限制字数,这本身就是错的。苏格拉底式教学也不是就代表只会问问题吧?也能提炼重点总结精华说要点,注重的是输出的质量」

**用户对**。我之前给三导师设了硬字数上限(卡帕西 ≤ 80 / 钱学森 ≤ 150 / 阿德勒 60-100),这是**对苏格拉底教学法的矮化**。
...
把它简化成「不灌输,只问问题」+ 字数硬限制,是**对教学法的误读**。

### 重大调整 (v0.4.5)

**1. 删除所有字数硬上限**

- `lib/prompts/mentor-personas.ts` 重写 system prompt:删除「≤ 80 字」「≤ 150 字」「≤ 60 字」等所有硬性字数规则
- `MENTOR_REPLY_LIMITS` / `MENTOR_OPENING_LIMITS` 常量**完全删除**
- `lib/agents/builders.ts` `validateMentorReply` **不再校验字数**
- `lib/langgraph/orchestrate.ts` tokenBudget 从 200/300 提到 1000-1500(给 LLM 足够空间)

**2. 改为通用「质量原则」(所有导师共享)**

```
1. 信息密度优先(一句话能讲清就一句话,该 500 字就 500 字,删废话)
2. 准确清晰(用具体例子 / 真实数字 / 类比,不打官腔)
3. 教学方法多元(提问 / 拆穿 / 举例 / 反驳 / 提炼 / 给判断 都可用)
4. 留参与空间(末尾必须有钩子让用户回应 —— 不靠短,靠结构)
```

**3. 校验改为结构性检查(不查字数)**

- 卡帕西:回复必须包含**参与钩子**(问号 / 反问 / 邀请性短语:「你看」「你试试」「再说说」等)
- 钱学森:回复必须**要么给具体下一步,要么留参与钩子**,不能两个都没有
- 阿德勒:**第一句不能直接谈学习任务**(必须先接住情绪 / 状态)

**4. 三导师差异化由人格特征体现,不靠字数**

- 卡帕西:直接尖锐 / 拒绝学术腔 / 用代码 / 类比说话 / 承认 hack
- 钱学森:中性克制 / 结构化(1) 2) 3)) / 落地导向 / 不抒情
- 阿德勒:先接情绪 / 节奏慢 / 多用短句和留白 / 不鸡汤

### 文档同步更新

- `MENTORS.md` v0.4.5 重大调整版头(含用户原话引用) + 三导师章节字数描述全部改写
- `PRD.md` §10.0 新增「通用质量原则」+ §10.1/§10.2/§10.3 删除字数条 → 人格特征条
- `CLAUDE.md` §9.1/§9.2/§9.3 发言长度规则改写为质量原则

### 代码改动

- `lib/prompts/mentor-personas.ts` 重写(删除字数常量 + system prompt 改写)
- `lib/agents/builders.ts` validateMentorReply 改为结构性校验 / buildRewriteRequest 不再传字数 / checkReplyLength 改为空壳兼容老 API
- `lib/langgraph/orchestrate.ts` tokenBudget 上调
