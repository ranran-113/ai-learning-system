...
```text
v0.1.4（已发）       │ #1 输出校验   #6 Prompt caching                      ✅
                    │
v0.1.5（已发）       │ 5E 教学法 + 锚定开场 + 祛魅时刻指令 + 会话累积模型     ✅
                    │
v0.1.6+（计划）       │ #2 Few-shot
                    │
v0.2 / MVP 上线后    │ #3 上下文压缩  #4 结构化决策  #5 工具调用  #7 自动评估
```

## 4.1 v0.1.5 新增能力 —— 5E 教学法 + 会话累积模型

### 5E 教学法（已实现）

详见 MENTORS.md §1.1。卡帕西和钱学森现在按 Engage → Explore → Explain → Elaborate → Evaluate 五阶段教学:

- 开场带「锚定段」(120-200 字),告诉用户今天要弄清楚什么 + 第一个问题
- 后续回到苏格拉底纯提问模式
- 用户卡 2 轮以上时主动「祛魅」:"所谓 X 其实就是 Y"

实现位置:`lib/prompts/mentor-personas.ts` + `lib/agents/builders.ts` 的 `buildOpeningMessage`

### 会话累积模型（部分实现,完整版 v0.2）

**问题**:一节课的概念可能需要几小时甚至几天才能想透。当前的"完成一节课"模型暗示了**短期完成态**,违反"沉浸式学习"哲学。

**v0.1.5 部分实现**:
- 同一节课的会话归档后,下次进入自动恢复(`resumeArchivedSession`)
- /learn 加深度感指示器代替时间感
- 文案:"建议 15-25 分钟,但深入想透不限时长"

**v0.2 完整版要做的**:
- 把"课"重新定义为「能力领域」,把"会话"做成一节课的多次累积事件
- UI 展示:「你在「有效提问」上累计聊了 N 次,沉淀了 K 条,理解度 X%」
- mentor 在跨会话续学时,基于前几次会话内容继续推进而非重新开始
- 「掌握」状态由用户输出沉淀的密度和质量动态评估(配合 #7 评估 harness)

设计文件:待开 `docs/session-accumulation-model.md`(v0.2 启动时建)
